...
import { switchMap, map, zip, of, exhaustMap } from 'rxjs';
 ...
 *  de la petición anterior, y tomar la especie (species) o starships,
...
(() =>{
    ...
    getRequest(`${SW_API}${CHARACTER_URL}`).pipe(
        ...
        switchMap(response => zip(
            of(response),
            getRequest(response.starships[0])
        )),
        map(([characterResponse, starshipResponse]) => ({characterResponse, starshipResponse}))
